...
// ページディレクトリ：512個のページテーブルを保持
pub type PD = Table<2, 21, PT>;
// ページディレクトリポインターテーブル：512個のページディレクトリを保持
pub type PDPT = Table<3, 30, PD>;
// ページマッピングレベル4：512個のPDPTを保持
...
// 割り込みハンドラのエントリポイントをアセンブリ言語で生成
macro_rules! interrupt_entrypoint {
    // リテラルを1つ取る -> $indexという変数名に
    ($index:literal) => {
...
// IDTの各エントリを表す構造体
// offset_hight offset_mid offset_low: 対応する割り込みが発生した際に
// 呼ばれる割り込みハンドラのアドレス
// attr: ハンドラの種類などの情報
#[repr(C, packed)]
#[allow(dead_code)]
...
impl Idt {
    pub fn new(segment_selector: u16) -> Self {
        ...
        let params = IdtrParameters {
            limit,                  // IDTのサイズ
            base: entries.as_ptr(), // IDTのベースアドレス
        };
        info!("Loading IDT: {params:?}");
        // SAFETY: This is safe since it loads valid IDT that is constructed
...
#[allow(dead_code)]
#[repr(C, packed)]
pub struct GdtParameters {
    limit: u16,
    base: *const Gdt,
}

//  セグメントセレクタのLSB3bitは別の情報を入れる場所
// GDTのインデックスは3bit 左シフト
